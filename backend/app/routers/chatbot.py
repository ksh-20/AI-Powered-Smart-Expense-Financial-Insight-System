...
def chatbot(
    ...
    if expenses:
        ...
        top_cats_str = ", ".join(
            [f"{cat}: ₹{amt:,.2f} ({amt/total_spent*100:.1f}%)" for cat, amt in sorted_cats[:4]]
        )
        ...
        user_context = (
            ...
            f"- Monthly Budget Limit: ₹{user.monthly_budget:,.2f} (Budget Set: {'Yes' if user.monthly_budget > 0 else 'No'})\n"
        )
    else:
        user_context = "User has not recorded any transactions yet."
